...
class PreProcessing:
    def __init__(self,path:str):
        self.special_tokens = ["<s>", "</s>","<unk>","<mask>"]
    ...
    def sentence_piece(self): 
       spm.SentencePieceTrainer.train(
           '--input=sample_txt.txt --model_prefix=m_model --vocab_size=32000 --user_defined_symbols=<s>,</s>,<mask>'
       )
    def sentence_token(self, txt:str):
        ...
        # print(sp.piece_to_id('<s>'),sp.piece_to_id('</s>'),sp.piece_to_id('<mask>'))
        self.mask_token_id = sp.piece_to_id('<mask>')
        return sp.encode_as_ids(processed_text)
